  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* ── ZONE GAUCHE : message ── */}
        <div className="flex-1 flex flex-col px-8 pt-10 pb-7 relative min-w-0">
          {/* Décors floraux */}
          <div className="absolute top-6 left-6 pointer-events-none select-none" style={{ fontSize: '28px', color: '#e8c4c4', opacity: 0.5 }}>❀</div>
          <div className="absolute bottom-6 left-6 pointer-events-none select-none" style={{ fontSize: '20px', color: '#e8c4c4', opacity: 0.4 }}>❀</div>
          <div className="absolute bottom-6 right-6 pointer-events-none select-none" style={{ fontSize: '18px', color: '#e8c4c4', opacity: 0.3 }}>❁</div>
          ...
          {/* Prénom */}
          <div className="pt-4 mt-2" style={{ borderTop: '1px solid rgba(200,160,60,0.25)' }}>
            <input
              type="text"
              value={senderName}
        ...
        {/* Séparateur vertical */}
        <div className="self-stretch my-10" style={{
          width: '1px',
        ...
        <div className="flex flex-col items-end px-6 pt-8 pb-7 gap-5" style={{ width: '170px', flexShrink: 0 }}>
          {/* Slot timbre */}
          <div
          ...
          >
            {selectedStamp ? (
              <motion.img
                ...
                style={{ width: '110px', height: '130px', objectFit: 'contain', filter: 'drop-shadow(2px 3px 6px rgba(0,0,0,0.25))' }}
              />
            ) : (
              <div className="text-center">
                <div style={{ fontSize: '30px', color: 'rgba(200,160,60,0.4)' }}>✉</div>
                <span className="font-serif" style={{ fontSize: '12px', color: 'rgba(200,160,60,0.5)' }}>timbre</span>
              </div>
            )}
          </div>

          {/* Lignes adresse */}
          <div className="flex-1 w-full flex flex-col justify-end gap-4">
            {[...Array(5)].map((_, i) => (
              <div key={i} className="w-full" style={{ height: '1px', background: 'rgba(200,160,60,0.3)' }} />
            ))}
            <div className="mt-2 text-right">
              <span className="font-handwriting text-rose-300/80 italic" style={{ fontSize: '14px' }}>avec amour 💌</span>
            </div>
          </div>
        </div>
      </div>
